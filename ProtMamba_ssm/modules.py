...
def decode_safe(
    ...
    if mask_dictionary:
        masks = list(mask_dictionary.keys())
        fim_lengths = [x[1] for x in list(mask_dictionary.values())]
        cur_fim_length = fim_lengths[0]
        cur_frag_length = 0
        cur_mask_ind = 0
    ...
    while not should_stop(sequences[-1], inference_params):
        ...
        if repetition_penalty == 1.0:
            ...
            if mask_dictionary and cur_frag_length == cur_fim_length:
                cur_frag_length = 0
                if cur_mask_ind < len(masks) - 1:
                    cur_mask_ind += 1
                    next_mask = masks[cur_mask_ind]
                    sampled_tokens = torch.tensor([[AA_TO_ID[next_mask]]]).to(input_ids.device)
                    cur_fim_length = fim_lengths[cur_mask_ind]
                else:
                    sampled_tokens = torch.tensor([[AA_TO_ID["<cls>"]]]).to(input_ids.device)
            else:
                sampled_tokens = sample_tokens(scores[-1], inference_params)
                if mask_dictionary and sampled_tokens.item() < 33:
                    cur_frag_length += 1
...
class GenerationMixinSafe(GenerationMixin):
    ...
    def generate(
            ...
            **kwargs):
        output = decode_safe(
            input_ids, position_ids, seq_position_ids, is_fim, self, max_length, top_k=top_k, top_p=top_p, min_p=min_p,
            mask_dictionary=mask_dictionary, temperature=temperature, **kwargs
        )
...
class MambaLMHeadModelSafe(nn.Module, GenerationMixinSafe):
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0, save_layer=[], *args,
                **kwargs):
        """
...
class MixerModelWith2DPosids(nn.Module):
    ...
    def forward(self, input_ids, position_ids, seq_position_ids, inference_params=None, save_layer=[]):
        hidden_states = torch.cat([self.embedding(input_ids), self.position_embedding(position_ids),
                                   self.seq_position_embedding(seq_position_ids), ], -1)
        residual = None
        if len(save_layer) > 0:
            hidden_states_dict = {}
        for i, layer in enumerate(self.layers):
...
class MambaLMHeadModelwithPosids(nn.Module, GenerationMixinSafe):
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0, save_layer=[],
                last_hidden_states=False, *args, **kwargs):
        """
        "position_ids" is just to be compatible with Transformer generation. We don't use it.
        num_last_tokens: if > 0, only return the logits for the last n tokens
        """
    ...
    def protected_forward(self, input_ids, position_ids=None, inference_params=None, num_last_tokens=0, save_layer=[],
                          last_hidden_states=False):
        hidden_states = self.backbone(input_ids, position_ids=position_ids, inference_params=inference_params,
...
class MambaLMHeadModelwith2DPosids(nn.Module, GenerationMixinSafe):
    ...
    def forward(self, input_ids, position_ids=None, seq_position_ids=None, inference_params=None, num_last_tokens=0,
                save_layer=[], *args, **kwargs):
        """
        "position_ids" is just to be compatible with Transformer generation. We don't use it.
        num_last_tokens: if > 0, only return the logits for the last n tokens
        """
        return self.protected_forward(input_ids, position_ids, seq_position_ids, inference_params, num_last_tokens,
                                      save_layer)

    def protected_forward(self, input_ids, position_ids=None, seq_position_ids=None, inference_params=None,
                          num_last_tokens=0, save_layer=[]):
        hidden_states = self.backbone(input_ids, position_ids=position_ids, seq_position_ids=seq_position_ids,
                                      inference_params=inference_params, save_layer=save_layer)
        if len(save_layer) > 0:
            return hidden_states
