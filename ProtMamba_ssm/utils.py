...
def generate_sequence(model, tokens, position_ids=None, seq_position_ids=None, is_fim=False, max_length=1,
                      temperature=1., top_p=0.0, top_k=1, mask_dictionary = None,
...
def prepare_tokens(context_tokens,
    ...
    if len(context_tokens) >= 1:
        seq, pos_ids = data_class.sample_sequences(context_tokens.cpu().numpy()[0], num_sequences=num_sequences, shuffle=shuffle)
        # convert to tensor and add batch dimension
        seq = torch.asarray(seq, dtype=torch.int64)[None,:].to(device)
...
def prepare_target(target, use_fim=None):
    ...
    if "<cls>" in use_fim:
        ...
        for mask in use_fim:
            ...
            mask_positions = (mask_positions[0]-cum_diff_length, mask_positions[1]-cum_diff_length)

            diff_length = mask_positions[1] - mask_positions[0]
            new_target = torch.cat([target[:,:mask_positions[0]],
                                    torch.full((target.shape[0], 1), AA_TO_ID[mask], dtype=torch.int64).to(device),
            ...
            pos_ids = new_pos_ids
            diff_length -= 1
            cum_diff_length += diff_length

        new_target = torch.cat([target,
                                torch.full((target.shape[0], 1), AA_TO_ID["<eos>"], dtype=torch.int64).to(device)], dim=1)
